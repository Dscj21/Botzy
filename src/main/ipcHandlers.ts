...
export const setupIpcHandlers = (mainWindow: BrowserWindow) => {
  ...
  ipcMain.handle('db:update-profile', (_, profile) => {
    ...
    if (shouldUpdateCard) {
        newEncryptedCard = encrypt(JSON.stringify({
             number: profile.cardNumber, expiry: profile.cardExpiry, cvv: profile.cardCvv
        }))
    }
    ...
    let stmt 
    if (shouldUpdateCard) {
         stmt = dbManager.getDb().prepare(`UPDATE profiles SET profile_name = @profile_name, address_json = @address_json, card_encrypted = @card_encrypted WHERE id = @id`)
         return stmt.run({ ...profile, card_encrypted: newEncryptedCard })
    } else {
         stmt = dbManager.getDb().prepare(`UPDATE profiles SET profile_name = @profile_name, address_json = @address_json WHERE id = @id`)
         return stmt.run({ ...profile, card_encrypted: undefined }) 
    }
  })

  ipcMain.handle('db:delete-account', (_, id) => dbManager.getDb().prepare('DELETE FROM accounts WHERE id = ?').run(id))
  ipcMain.handle('db:delete-profile', (_, id) => dbManager.getDb().prepare('DELETE FROM profiles WHERE id = ?').run(id))
  ...
  ipcMain.handle('session:open', async (_, { accountId, url, background }) => {
      const account = dbManager.getDb().prepare('SELECT * FROM accounts WHERE id = ?').get(accountId) as any
      if (!account) return
      ...
      if (account.profile_id) {
          const profile = dbManager.getDb().prepare('SELECT * FROM profiles WHERE id = ?').get(account.profile_id) as any
  ...
  autoUpdater.on('update-download-progress', (progress) => mainWindow.webContents.send('update-download-progress', progress))
  ...
  ipcMain.handle('automation:empty-cart', (_, id) => sessionManager.runAutomation(id.toString(), 'empty-cart'))
  ipcMain.handle('automation:add-to-cart', (_, { id, url }) => sessionManager.runAutomation(id.toString(), 'add-to-cart', { url }))
  ipcMain.handle('automation:camp-checkout', (_, id) => sessionManager.runAutomation(id.toString(), 'camp-checkout'))
  ...
  ipcMain.handle('db:import-all', async () => {
      ...
      try {
          ...
          if (!data.accounts || !data.profiles) throw new Error('Invalid backup format: Missing accounts or profiles')
  ...
  ipcMain.handle('automation:save-vcc', async (event, data) => {
      ...
      try {
          ...
          const rect = data.rect ? { x: Math.round(data.rect.x), y: Math.round(data.rect.y), width: Math.round(data.rect.width), height: Math.round(data.rect.height) } : undefined
  ...
  ipcMain.on('automation:export-vccs', (event) => {
      ...
      try {
          if (!fs.existsSync(vccPath)) {
             event.sender.send('automation:export-vccs-reply', { success: false, error: 'No cards found' });
             return;
          }
          const data = JSON.parse(fs.readFileSync(vccPath, 'utf-8'));
          const header = 'Name,Number,CVV,Expiry,Amount,Date\n';
          const rows = data.map((c: any) => `${c.name},${c.number},${c.cvv},${c.expiry},${c.amount},${c.generated_at}`).join('\n');
