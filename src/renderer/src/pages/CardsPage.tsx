...
import { Play, CreditCard, Trash2, Save, List, FileSpreadsheet, RefreshCcw, Copy, Loader2, Moon, Sun, Zap, Calendar, Search, Eye, X } from 'lucide-react'
...
export function CardsPage() {
    ...
    // Effects
    useEffect(() => {
        const removeLog = (window as any).electron.ipcRenderer.on('automation:log', (_e: any, msg: string) => {
            setLogs(prev => [...prev.slice(-19), msg])
        })
        const removeProgress = (window as any).electron.ipcRenderer.on('automation:progress', (_e: any, data: any) => {
            setProgress(data)
            if (activeTab === 'history') fetchHistory()
        })
        return () => { removeLog(); removeProgress(); }
    }, [activeTab])
        ...
        try {
            ...
            if (saved) {
                ...
                if (old) {
                    ...
                    if (p.username) setAccounts([{ bank: 'hdfc', username: p.username, password: p.password }])
                }
            }
    ...
    const handleDelete = async (ids: string[]) => {
        if (!confirm(`Are you sure you want to delete ${ids.length} card(s)? This action cannot be undone.`)) return
    ...
    const filteredHistory = history.filter(h => {
        const matchesSearch = (h.number || '').includes(searchTerm) || (h.amount || '').includes(searchTerm)
        const matchesDate = !dateFilter || (h.created_at || h.generated_at || '').startsWith(dateFilter)
        return matchesSearch && matchesDate
    })

    const handleStart = () => {
        localStorage.setItem('netsafe_last_config', JSON.stringify({
            amount, count, cardholderName, beneficiaryName, cardPassword,
            email, confirmEmail, mobile, confirmMobile, message
        }))
        ...
        ipc.send('automation:run', 'create-netsafe', {
            amount, count, bank, username, password,
            cardholderName, beneficiaryName, cardPassword, email, confirmEmail, mobile, confirmMobile, message
        })
    }
    ...
    const labelClass = isLight ? 'block text-xs font-medium text-slate-600 mb-1' : 'block text-xs font-medium text-zinc-400 mb-1'

    // Header Icon
    const iconContainerClass = isLight ? 'bg-white border border-slate-200 shadow-sm text-orange-600' : 'bg-green-500/10 text-green-500'
    ...
    const tabActive = isLight ? 'bg-white text-blue-600 shadow-sm border border-slate-200' : 'bg-green-600 text-white'
    const tabInactive = isLight ? 'text-slate-500 hover:text-slate-800 hover:bg-white/50' : 'text-zinc-400 hover:text-white'
            ...
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className={`text-sm ${isLight ? 'text-slate-500' : 'text-zinc-400'}`}>Generate and manage HDFC Netsafe cards</p>
                    </div>
                </div>

                <div className={`flex items-center gap-1 p-1 rounded-lg ${isLight ? 'bg-slate-200/50' : 'bg-white/5'}`}>
                    <button
                    ...
                    <div className={`ml-2 pl-2 border-l ${isLight ? 'border-slate-300' : 'border-white/10'} flex items-center gap-2`}>
                        {theme === 'dark' ? <Moon className="w-4 h-4 text-blue-400" /> : <Sun className="w-4 h-4 text-orange-500" />}
                        <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="text-xs font-medium hover:underline">
                            Theme
                        </button>
                    </div>
                </div>
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        ...
                        <div className={`p-6 rounded-xl ${cardClass}`}>
                            <div className="flex items-center justify-between mb-4">
                                <h2 className={`text-lg font-semibold ${isLight ? 'text-blue-700' : 'text-green-400'}`}>Run Automation</h2>
                            </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className={inputClass} placeholder="HDFC User ID"
                                        />
                                    </div>
                                    <div>
                        ...
                        <div className={`p-6 rounded-xl ${cardClass}`}>
                            <div className="grid grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className={inputClass} placeholder="Your Name on Account"
                                    />
                                </div>
                                <div>
                                    ...
                                    <input
                                        ...
                                        className={inputClass} placeholder="Any Name (e.g. john)"
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        value={cardPassword} onChange={e => setCardPassword(e.target.value)}
                                        className={inputClass} placeholder="VCC Login Password"
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className={inputClass} placeholder="Email for OTP"
                                    />
                                </div>
                                <div>
                                    ...
                                    <input
                                        ...
                                        className={inputClass} placeholder="Confirm Email"
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className={inputClass} placeholder="Mobile for OTP"
                                    />
                                </div>
                                <div>
                                    <label className={labelClass}>Confirm Mobile</label>
                                    <input
                                        value={confirmMobile} onChange={e => setConfirmMobile(e.target.value)}
                                        className={inputClass} placeholder="Confirm Mobile"
                                    />
                                </div>
                                ...
                                <div className="col-span-2">
                                    ...
                                    <input
                                        ...
                                        className={inputClass} placeholder="Message"
                                    />
                                </div>
                            ...
                            <div className="mt-8">
                                <button
                                    onClick={handleStart}
                                    disabled={isRunning}
                                    className={`w-full py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg ${isRunning ? 'bg-zinc-600 cursor-not-allowed opacity-50' : primaryBtnClass}`}
                                >
                                    {isRunning ? <Loader2 className="w-5 h-5 animate-spin" /> : <Play className="w-5 h-5 fill-current" />}
                                    {isRunning ? `Running... (${progress.generated}/${progress.total})` : 'Start VCC Generation'}
                                </button>
                                {isRunning && <div className="text-center text-xs text-zinc-500 mt-2 animate-pulse">{progress.status}</div>}
                            </div>
                        </div>
                        ...
                        {isRunning && (
                            <div className="bg-black rounded-xl p-4 font-mono text-xs h-48 overflow-y-auto border border-white/10 shadow-inner">
                                ...
                                <div className="space-y-1">
                                    {logs.map((L, i) => (
                                        <div key={i} className="text-zinc-300 border-l-2 border-zinc-800 pl-2">{L}</div>
                                    ))}
                                </div>
                            </div>
                        )}
                    ...
                    <div className="lg:col-span-4">
                        <div className={`p-6 rounded-xl ${cardClass} h-full`}>
                            ...
                            <div className="space-y-3">
                                {accounts.length === 0 && <div className="text-sm text-zinc-500 italic">No saved accounts yet.</div>}
                                {accounts.map((acc, i) => (
                                    <div key={i} className={`p-3 rounded-lg border flex items-center justify-between group transition-all ${isLight ? 'bg-slate-50 border-slate-200 hover:border-orange-300' : 'bg-white/5 border-white/5 hover:border-green-500/30'}`}>
                                        <div
                ...
                <div className={`p-6 rounded-xl ${cardClass} flex flex-col h-full`}>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                        <div className="flex items-center gap-2">
                            <h2 className={`text-xl font-semibold flex items-center gap-2 ${isLight ? 'text-blue-700' : ''}`}>
                        ...
                        <div className="flex items-center gap-2 w-full md:w-auto">
                            {/* Filters */}
                            <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border ${isLight ? 'bg-white border-slate-200' : 'bg-black/20 border-white/10'}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        <div className={`p-4 rounded-xl border ${isLight ? 'bg-blue-50 border-blue-100' : 'bg-black/40 border-white/5'}`}>
                            <div className="text-zinc-500 text-xs uppercase tracking-wider mb-1">Total Cards Generated</div>
                            <div className={`text-2xl font-bold ${isLight ? 'text-blue-700' : 'text-white'}`}>{history.length}</div>
                        </div>
                        <div className={`p-4 rounded-xl border ${isLight ? 'bg-green-50 border-green-100' : 'bg-black/40 border-white/5'}`}>
                            <div className="text-zinc-500 text-xs uppercase tracking-wider mb-1">Total Value</div>
                            <div className="text-2xl font-bold text-green-500">₹{history.reduce((acc, curr) => acc + (Number(curr.amount) || 0), 0).toLocaleString()}</div>
                        </div>
                        <div className={`p-4 rounded-xl border ${isLight ? 'bg-orange-50 border-orange-100' : 'bg-black/40 border-white/5'}`}>
                            <div className="text-zinc-500 text-xs uppercase tracking-wider mb-1">Selected Value</div>
                            <div className="text-2xl font-bold text-orange-500">
                                ₹{history.filter(h => selectedIds.includes(h.number)).reduce((acc, curr) => acc + (Number(curr.amount) || 0), 0).toLocaleString()}
                            </div>
                        </div>
                    </div>
                    ...
                    {loadingHistory ? (
                        ...
                        <div className="flex-1 overflow-auto rounded-lg border border-slate-200 dark:border-white/5">
                            <table className="w-full text-left text-sm relative">
                                <thead className={`sticky top-0 z-10 ${isLight ? 'bg-slate-50 text-slate-500 border-b border-slate-200' : 'bg-black/90 text-zinc-400 border-b border-white/10'}`}>
                                    <tr>
                                ...
                                <tbody className={`divide-y ${isLight ? 'divide-slate-200' : 'divide-white/5'}`}>
                                    {filteredHistory.map((item, i) => (
                                        <tr key={i} className={`transition-colors ${selectedIds.includes(item.number) ? (isLight ? 'bg-blue-50' : 'bg-blue-900/10') : ''} ${isLight ? 'hover:bg-slate-50' : 'hover:bg-white/5'}`}>
                                            <td className="p-4">
                                                <input
                                                    type="checkbox"
                                                    className="rounded border-gray-300"
                                            ...
                                            <td className="p-4">
                                                <span className={`px-2 py-1 rounded-full text-xs ${item.status === 'Unused' ? (isLight ? 'bg-green-100 text-green-700' : 'bg-green-500/20 text-green-400') : (isLight ? 'bg-red-100 text-red-700' : 'bg-red-500/20 text-red-400')}`}>
                                                    {item.status || 'Unused'}
                                                </span>
                                            </td>
                                            <td className="p-4 flex gap-2 justify-end">
                                                <button title="Copy" onClick={() => copyToClipboard(`${item.number}|${item.expiry}|${item.cvv}`)} className={`p-2 rounded hover:text-blue-500 ${isLight ? 'hover:bg-slate-200' : 'hover:bg-white/10'}`}>
                                                    <Copy className="w-4 h-4" />
                                                </button>
                                                <button title="Delete" onClick={() => handleDelete([item.number])} className={`p-2 rounded hover:text-red-500 ${isLight ? 'hover:bg-slate-200' : 'hover:bg-white/10'}`}>
                                                    <Trash2 className="w-4 h-4" />
                                                </button>
                                            </td>
                                        </tr>
                    ...
                    {viewSnapshot && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={() => setViewSnapshot(null)}>
                            <div className="relative max-w-4xl max-h-full" onClick={e => e.stopPropagation()}>
                                <button className="absolute -top-10 right-0 text-white hover:text-zinc-300" onClick={() => setViewSnapshot(null)}>
                                    <X className="w-8 h-8" />
                                </button>
                                <img src={`file://${viewSnapshot}`} className="max-w-full max-h-[85vh] rounded shadow-2xl border-4 border-white/10" />
