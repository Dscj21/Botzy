...
function dispatchInputEvent(element: any) {
  // Use any to avoid lint errors if HTMLElement not ready
  if (!element) return
  try {
...
function sendLog(message) {
  try {
    ipcRenderer.send('automation:log', message)
  } catch (e) {}
}
...
function getQueue() {
  return JSON.parse(sessionStorage.getItem(Q_KEY) || '[]')
}
function setQueue(q) {
...
function addDone(url) {
  ...
  if (!d.includes(url)) {
    d.push(url)
    sessionStorage.setItem(DONE_KEY, JSON.stringify(d))
  }
}
...
async function startCheckoutLoop() {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (placeOrderCandidates.length > 0) {
      ...
      sendLog(
        `[Auto] > Place Order (Attempt ${window['placeOrderAttempts']}) - Found ${placeOrderCandidates.length} btns`
      )
    ...
    const deliverHere = allBtns.find(
      (b) =>
        ((b as HTMLElement).innerText || '').toUpperCase().includes('DELIVER HERE') &&
    ...
    const paymentHeader = Array.from(document.querySelectorAll('div, span, label')).find((el) =>
      ((el as HTMLElement).innerText || '').includes('Credit / Debit / ATM Card')
    )
          ...
          if (!window['card_fetching']) {
            ...
            try {
              ...
              if (vcc) {
                ...
                if (cvvInput) {
                  cvvInput.value = vcc.cardCVV
                  dispatchInputEvent(cvvInput)
                }
                ...
                const payBtn = Array.from(document.querySelectorAll('button')).find((b) =>
                  (b.innerText || '').toUpperCase().includes('PAY')
                )
                if (payBtn) {
...
function clickSeeAllUpdates() {
  try {
    ...
    const target = candidates.find(
      (el) =>
        (el as HTMLElement).innerText && (el as HTMLElement).innerText.trim() === 'See All Updates'
    )
    if (target) {
...
function scrapeDetailDOM() {
  ...
  const strictTrackMatch = bodyText.match(
    new RegExp(`\\b(?:${courierIds.join('|')})[A-Za-z0-9]{8,}`, 'i')
  )
  ...
  let status = 'Ordered'
  if (
    bodyText.includes('Not Placed') ||
    bodyText.includes('Payment Failed') ||
    bodyText.includes('Order Not Placed')
  )
    status = 'Failed'
  else if (bodyText.includes('Delivered')) status = 'Delivered'
  else if (bodyText.includes('Cancelled')) status = 'Cancelled'
  else if (bodyText.includes('Returned')) status = 'Returned'
  else if (
    bodyText.includes('Shipped') ||
    bodyText.includes('Out for Delivery') ||
    bodyText.includes('Arriving')
  )
    status = 'Shipped'
  else if (bodyText.includes('Return') && !bodyText.includes('Return Policy')) status = 'Returned'
  ...
  const ordRegex =
    /Order Confirmed(?:,| on)\s+(?:[A-Za-z]{3}\s+)?([A-Za-z]{3}\s\d{1,2}(?:,?\s\d{4})?)/i
  ...
  // Priority 1: Specific Flipkart Selectors
  ...
  if (!name) {
    ...
    if (candidates.length > 0) {
      ...
      const img =
        candidates[0].el.querySelector('img') ||
        candidates[0].el.parentElement?.querySelector('img') ||
        candidates[0].el.parentElement?.parentElement?.querySelector('img')
  ...
  if (!imageUrl) {
    ...
    const prodImg = imgs.find(
      (img) =>
        img.clientWidth > 50 &&
        img.clientHeight > 50 &&
        img.closest('div') &&
        !img.alt.includes('Flipkart')
    )
  ...
  if (!name || name.startsWith('Product OD')) {
    ...
    const textNodes = allDivs
      .filter(
        (d) =>
          (d as HTMLElement).children.length === 0 &&
          (d as HTMLElement).innerText.length > 15 &&
          (d as HTMLElement).innerText.length < 100
      )
...
async function runCrawler() {
  ...
  if (url.includes('account/orders') || url.includes('order-history')) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const badName =
        !order ||
        order.product_name.startsWith('Product OD') ||
        order.product_name.includes(',') ||
        order.product_name.includes('Your item') ||
        order.product_name.includes('Payment')
...
function sendProgress(generated, total, status) {
  try {
    ipcRenderer.send('automation:progress', { generated, total, status })
  } catch (e) {}
}

async function runNetsafeLoop(cfg) {
  // Session state
  const total = parseInt(cfg.count) || 1
  ...
  if (document.querySelector('input[type="password"]')) {
    ...
    if (!window['netsafe_log_login']) {
      window['netsafe_log_login'] = true
      sendLog('[Netsafe] Login Page Detected')
    }
    ...
    if (cfg.username && cfg.password) {
      const inputs = Array.from(document.querySelectorAll('input')).filter((el) => {
        ...
        return (
          ['text', 'password', 'number', 'email', 'tel'].includes(t) && el.offsetParent !== null
        )
      })
  ...
  const allBtns = getDeepElements(
    'button, input[type="button"], input[type="submit"], input[type="image"], a, div[role="button"]'
  )
  goBtn = allBtns.find((b) => {
    const t = (
      ...
      .toLowerCase()
    return (
      (t === 'go' || t.includes('go.gif') || t.includes('go.jpg') || t === 'submit') &&
      b.offsetParent !== null
    )
  }) as HTMLElement | null
  ...
  if (goBtn) {
    ...
    const localInputs = Array.from(
      goBtn.ownerDocument.querySelectorAll('input:not([type="hidden"]):not([disabled])')
    ) as HTMLInputElement[]
  ...
  if (!amountInp) {
    const allInputs = getDeepElements(
  ...
  if (goBtn && amountInp && amountInp.value && !window['netsafe_go_clicked']) {
    ...
    setTimeout(() => {
      window['netsafe_go_clicked'] = false
    }, 5000)
  }

  // 3. Check "Agree" Button (Terms & Conditions)
  const agreeBtn = getDeepElements('a, button, input[type="button"], input[type="submit"]').find(
    (b) => {
  ...
  if (formFound && !window['netsafe_form_submitted']) {
    ...
    const intervalId = setInterval(() => {
      ...
      setVal(
        ['input[name="txtReenterEmail"]'],
        'Confirm Beneficiary Email',
        cfg.confirmEmail || cfg.email
      )
      ...
      const mobileHandler = (selectors: string[], label: string, val: string) => {
        ...
        if (el) {
          ...
          if ((r.width > 0 && r.width < 60) || maxLen < 10) {
            ...
            while (next) {
              const nextInp =
                next.querySelector('input') || (next.tagName === 'INPUT' ? next : null)
      ...
      mobileHandler(
        ['input[name="txtBeneficiaryMobile"]', 'input[name="txtMobileNo"]'],
        'Beneficiary Mobile',
        cfg.mobile
      )
      mobileHandler(
        ['input[name="txtReenterBeneficiaryMobile"]', 'input[name="txtReenterMobileNo"]'],
        'Confirm Beneficiary Mobile',
        cfg.confirmMobile || cfg.mobile
      )

      // 5. Message
      setVal(
        ['input[name="txtMessage"]', 'textarea[name="txtMessage"]'],
        'Message',
        cfg.message || 'Gift'
      )
      ...
      setVal(
        ['input[name="txtReenterPassword"]'],
        'Confirm Password',
        cfg.cardPassword || cfg.password,
        true
      )
    }, 300)
    ...
    let sendBtn = targetDoc.querySelector(
      'input[type="image"][alt="Send"], input[src*="send.gif"], input[src*="submit.gif"]'
    ) as HTMLElement
    if (!sendBtn) {
      sendBtn = Array.from(
        targetDoc.querySelectorAll('a, button, input[type="button"], input[type="submit"], img')
      ).find((b) => {
        const t = (
        ...
        return (
          (t.includes('send') || t.includes('submit') || t === 'go') &&
          (b as HTMLElement).offsetParent !== null
        )
      }) as HTMLElement
    }
    ...
    if (!window['cardSaved']) {
      ...
      if (generated < total) {
        ...
        const backBtn = getDeepElements('a, button, input').find((b) => {
          ...
          return (
            (t.includes('back') ||
              t.includes('create another') ||
              (t.includes('generate') && !t.includes('ivr'))) &&
            !t.includes('log') &&
            !t.includes('profile')
          )
        }) as HTMLElement
...
async function aggressiveClick(el, depth = 0) {
  ...
  try {
    ...
    if (rect.width > 0 && rect.height > 0) {
      try {
        await ipcRenderer.invoke('automation:click-at', { x, y })
      } catch (e) {}
      await new Promise((r) => setTimeout(r, 50))
      try {
        await ipcRenderer.invoke('automation:click-at', { x: x - 5, y: y - 5 })
      } catch (e) {}
    }
  ...
  try {
    if (command === 'empty-cart') {
      ...
      for (let i = 0; i < 15; i++) {
        ...
        let confirmBtn = Array.from(document.querySelectorAll('div, button, span')).find((el) => {
          const text = ((el as HTMLElement).innerText || '').trim().toLowerCase()
          return (
            text === 'remove' && el !== removeBtn && el.isConnected && el.closest('div._3dsJAO')
          )
        })
        if (!confirmBtn)
          confirmBtn = document.querySelector('div._3dsJAO._24d-qY.FhkMJZ') || undefined
        if (confirmBtn) {
    ...
    if (command === 'add-to-cart') {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        ...
        const goBtn = btns.find((b) =>
          ((b as HTMLElement).innerText || '').toUpperCase().includes('GO TO CART')
        )
        if (goBtn) {
          success = true
          break
        ...
        if (candidates.length > 0) {
          ...
          for (let k = 0; k < 12; k++) {
            ...
            const foundGo = checkBtns.find((b) =>
              ((b as HTMLElement).innerText || '').toUpperCase().includes('GO TO CART')
            )
            if (foundGo) {
              success = true
          ...
          const skipBtn = btns.find((b) => {
            const t = ((b as HTMLElement).innerText || '').toUpperCase()
            return (
              (t === 'SKIP' || t === 'CONTINUE' || t === 'SAVE & CONTINUE') &&
              (b as HTMLElement).offsetParent !== null
            )
          })
    ...
    if (command === 'update-profile-auto') {
      ...
      if (window.location.href.includes('address')) {
        const addBtn = Array.from(document.querySelectorAll('div, button')).find(
          (el) => (el as HTMLElement).innerText?.toUpperCase() === 'ADD A NEW ADDRESS'
        )
        if (addBtn) {
          await aggressiveClick(addBtn as HTMLElement)
          await new Promise((r) => setTimeout(r, 2000))
        ...
        const fillField = (identifiers: string[], val: string, isTextarea = false) => {
          ...
          const el = allInputs.find((i) => {
            ...
            if (i.previousElementSibling)
              labelText += (i.previousElementSibling as HTMLElement).innerText || ''
            if (i.parentElement?.previousElementSibling)
              labelText += (i.parentElement.previousElementSibling as HTMLElement).innerText || ''

            labelText = labelText.toLowerCase()
            return identifiers.some(
              (id) => t.includes(id) || p.includes(id) || labelText.includes(id)
            )
        ...
        const saveBtn = Array.from(document.querySelectorAll('button')).find(
          (b) => b.innerText.toUpperCase() === 'SAVE'
        )
    ...
    if (command === 'camp-checkout') {
      ...
      const visibleCheckoutBtn = allBtns.find((b) => {
        const t = ((b as HTMLElement).innerText || '').toUpperCase()
        return (
          (t.includes('PLACE ORDER') || t.includes('DELIVER HERE') || t.includes('CONTINUE')) &&
          (b as HTMLElement).offsetParent !== null
        )
      })
